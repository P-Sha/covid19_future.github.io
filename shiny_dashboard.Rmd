...
ctest = read_csv("./data/covid19_global.csv") %>% 
  ...
  select(name, covid_cases)

quant_10 = ctest %>% pull(covid_cases) %>% quantile(probs = seq(0,1,.1)) %>% as.vector() %>% round()

covid_countries_map = sp::merge(x = countries, y = ctest, by = "name", all.x = TRUE)

pal = colorBin(palette = "YlOrRd", bins = quant_10, na.color = "#808080")
```

Column {.sidebar}
-----------------------------------------------------------------------
```{r shiny_inputs}
# borough_choices = ctest %>% pull(neighbourhood_group) %>% unique()
# 
# selectInput(
#   "borough_choice",
#   h3("Borough Choice"),
...
covid_countries_map %>% 
  leaflet() %>%
  addTiles() %>%
  addPolygons(fillColor = ~pal(covid_cases),
  weight = 1.5,
  opacity = 1,
  color = "white",
  dashArray = "3",
...
```{r}
ctest %>% 
  mutate(name = fct_reorder(name, covid_cases)) %>% 
  plot_ly(x = ~covid_cases,
         y = ~name,
         type = "bar",
         colors = "viridis")
```
